...
No **pre-push** hooks are adopted: v2.1.0's `pre-push-ts.yml` fragment gates pushes on the test suite (`pnpm test`, overridden to `npm test` for npm repos), but this repo has no `test` script (and no standalone `typecheck` script) in `package.json`, so consuming it would fail every push. Type-checking is covered by `npm run build` (`tsc -noEmit -skipLibCheck`) in CI. If a test suite is ever added, consume `pre-push-ts.yml` with a `test-ts: run: npm test` override in `lefthook-local.yml`.
